...
function DroppableRegion({ region, isActive }: { region: Region; isActive: boolean }) {
                ...
                <div className="space-y-4 w-full">
                    <div className="relative w-full aspect-square overflow-hidden">
                        ...
                        {Object.entries(placements).map(([regionName, labelIndex]) => {
                            ...
                            const { x: cx, y: cy } = getPolygonCentroid(region.points);
                            ...
                            return (
                                <div
                                    key={regionName}
                                    className={`
                                        absolute text-sm font-bold select-none px-3 py-1 rounded border border-white
                    ...
                    <div className="flex flex-wrap justify-center gap-2 pt-2">
                        {exercise.labels.map((label, idx) => {
                            const id = `tray-${idx}`;
                            const isPlaced = placedLabelRegion[idx] !== undefined;
                            const isBeingDragged = activeId === id;

                            return !isPlaced && !isBeingDragged ? (
                                <DraggableLabel key={id} id={id}>
                                    {label.text[lang]}
                                </DraggableLabel>
                            ) : null;
                        })}
